          ...
          <div className="flex items-center flex-1 justify-center px-8 lg:px-12 space-x-3 lg:space-x-6">
            ...
            {navItems.slice(0, 3).map((item) => (
              <NavLink
                ...
                to={item.path}
                className={({ isActive }) =>
                  `flex items-center px-3 lg:px-4 py-2 rounded-xl text-sm font-medium transition-all whitespace-nowrap ${isActive
            ...
            {/* Tabs Derecha (Incluye Visitas como botón normal) */}
            {navItems.slice(3).map((item) => (
              <NavLink
                key={item.name}
                to={item.path}
                className={({ isActive }) =>
                  `flex items-center px-3 lg:px-4 py-2 rounded-xl text-sm font-medium transition-all whitespace-nowrap relative ${isActive
                    ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/30'
                    : 'text-blue-100 hover:bg-blue-800/80 hover:text-white'
                  }`
                }
